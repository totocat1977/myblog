...
class MyUser(AbstractUser):
    ...
    '''
    mbu_is_login = models.BooleanField('Login Status', default=False)
    mbu_status = models.BooleanField('User Status', default=True)
    #mbu_email = models.EmailField('Email Address')
    mbu_avatar = models.ImageField(upload_to='uploads/avatars/%Y/%m/%d/%H/%M/%S', default='uploads/avatars/default_avatar.jpg', verbose_name='Avatar')
...
class Category(models.Model):
    ...
    mbc_name = models.CharField('Category Name', max_length=20, null=False, blank=False)
    mbc_desc = models.CharField('Category Description', max_length=50, default='')
    # mbc_parent_id = models.SmallIntegerField('Parent Category ID', default=0)
    mbc_parent_id = models.ForeignKey('self', on_delete=models.CASCADE, db_column='mbc_parent_id', verbose_name='Parent Category ID', related_name='subcategory')
    mbc_order = models.SmallIntegerField('Category Orders', blank=False, default=0)
...
class Post(models.Model):
	...
	mbp_image = models.ImageField('Post Image', upload_to='uploads/postimg/%Y/%m/%d/%H/%M/%S', default='uploads/postimg/default_post.jpg')
	mbp_image_caption = models.CharField('Image Caption', max_length=16, default='')
	#mbp_auther_id = models.models.IntegerField()
	mbp_auther_id = models.ForeignKey(MyUser, to_field='id', on_delete=models.CASCADE, related_name='auther', db_column='mbp_auther_id', verbose_name='Auther ID')
	mbp_auther = models.CharField('Auther Name', max_length=20)
	mbp_created_at = models.DateTimeField('Post Create DateTime', auto_now_add=True)
	mbp_mod_flag = models.BooleanField('Post modify flag', default=False)
	mbp_last_at = models.DateTimeField('Post last modify datetime', auto_now=True)
	#mbp_last_by_id = models.IntegerField()
	mbp_last_by_id = models.ForeignKey(MyUser, to_field='id', on_delete=models.CASCADE, related_name='editor', db_column='mbp_last_by_id', verbose_name='Last Modified By')
	mbp_last_by_name = models.CharField('Post last modify name', max_length=20)
	#mbp_category = models.IntegerField('Post Category ID')
	mbp_category = models.ForeignKey(Category, to_field='mbc_id', on_delete=models.CASCADE, db_column='mbp_category', verbose_name='Category ID')
	...
	def get_all_tags(self):
		...
		for t in self.post.all():
			r.append([t.mbp_t_tid.mbt_name,t.mbp_t_tid.mbt_colormark])
		return r
	...
	def month_list():
		...
		year_month_number = []
		for key in counter:
			year_month_number_c.append([key[0],key[1],month_map[key[1]-1],counter[key]])
...
class Comment(models.Model):
	...
	mbc_auther_id = models.ForeignKey(MyUser, to_field='id', on_delete=models.CASCADE, db_column='mbc_auther_id', verbose_name='Auther ID') 
	...
	mbc_post_id = models.ForeignKey(Post, to_field='mbp_id', on_delete=models.CASCADE, db_column='mbc_post_id', verbose_name='Post ID')
	#mbc_reply_id = models.CharField('Replied Comment ID', max_length=8, default='0000')
	mbc_reply_id = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, db_column='mbc_reply_id', verbose_name='Comment ID')
...
class Post_Static(models.Model):
	...
	mbp_s_pid = models.OneToOneField(Post, to_field='mbp_id', on_delete=models.CASCADE, primary_key=True, db_column='mbp_s_pid', related_name='post_static', verbose_name='Post ID')
...
class Post_Tag(models.Model):
    ...
    mbp_t_pid = models.ForeignKey(Post, to_field='mbp_id', on_delete=models.CASCADE, db_column='mbp_t_pid', related_name='post', verbose_name='Post ID')
    mbp_t_tid = models.ForeignKey(Tag, to_field='mbt_id', on_delete=models.CASCADE, db_column='mbp_t_tid', related_name='tag', verbose_name='Tag ID')
    mbp_t_created_at = models.DateTimeField('Link Create DateTime', auto_now_add=True)
